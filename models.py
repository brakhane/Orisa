...
from sqlalchemy import (Boolean, Column, DateTime, BigInteger, Integer, SmallInteger, String,
...
class BattleTag(Base):
    ...
    sr_history = relationship("SR", order_by="desc(SR.timestamp)", cascade="all, delete-orphan", lazy="dynamic",
                              foreign_keys="SR.battle_tag_id", back_populates="battle_tag")
    current_sr = relationship("SR", uselist=False, foreign_keys=[current_sr_id], post_update=True, lazy="joined")
    ...
    def update_sr(self, new_sr, *, timestamp=None):
        ...
        if len(self.sr_history[:2]) > 1 and self.sr_history[0].value == self.sr_history[1].value:
            sr_obj = self.sr_history[0]
            sr_obj.timestamp = timestamp
            sr_obj.value = new_sr
        else:
            sr_obj = SR(timestamp=timestamp, value=new_sr)
            self.sr_history.append(sr_obj) # sqlalchemy dynamic wrapper does not support prepend
...
class SR(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    battle_tag_id = Column(Integer, ForeignKey('battle_tags.id'), nullable=False, index=True)

    battle_tag = relationship("BattleTag", back_populates="sr_history", foreign_keys=[battle_tag_id])
