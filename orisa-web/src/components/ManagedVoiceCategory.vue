  ...
  <div class="card mb-3 bg-light">
    <div class="card-header">
      <b-input-group prepend="Category">
        <b-form-select :state="val_state(validation_errors.category_id)" v-model="category.category_id">
          <template v-for="chan in channels">
            <option :key="chan.id" :value="chan.id" v-if="chan.type == 4">{{ chan.name }}</option>
          </template>
        </b-form-select>
        <b-input-group-append>
          <b-btn variant="outline-danger" @click="$emit('delete-cat-clicked')"><font-awesome-icon :icon="['far', 'trash-alt']"></font-awesome-icon></b-btn>
        </b-input-group-append>
        <div class="invalid-feedback" v-if="validation_errors.category_id">{{ validation_errors.category_id }}</div>
      </b-input-group>
    </div>
    <div class="card-body">
      <b-card header="Settings">
        ...
        >
          <b-form-input :state="val_state(validation_errors.channel_limit)" class="col-4 col-md-3" v-model.number="category.channel_limit" type="number" min="2" max="99"></b-form-input>
        </b-form-group>
      </b-card>
      <hr class="hr-4">
      <b-card header="Managed prefixes" no-body>
        <b-card-body>
          <b-alert variant="warning" :show="category.prefixes.length == 0">There are no prefixes configured yet!</b-alert>
          <div class="row bg-light m-2 p-2" :key="index" v-for="(prefix, index) in category.prefixes">
            <b-form-group
            ...
            >
              <b-form-input :state="val_state(val_errors_prefixes(index).name)" v-model="prefix.name" placeholder="e.g. Comp, or Quick Play"></b-form-input>
            </b-form-group>
            <b-form-group
            ...
            >
              <b-form-input :state="val_state(val_errors_prefixes(index).limit)" v-model.number="prefix.limit" type="number" min="0" max="99"></b-form-input>
            </b-form-group>
            <b-btn variant="outline-danger" @click="delete_prefix(category, index)"><font-awesome-icon :icon="['far', 'trash-alt']"></font-awesome-icon></b-btn>
          </div>
        </b-card-body>
        <b-card-body>
          <b-btn variant="primary" @click="add_prefix(category)"><font-awesome-icon icon="plus"/> Add a managed channel prefix</b-btn>
        </b-card-body>
      </b-card>
    </div>
  </div>
...
export default {
  ...
  methods: {
    val_errors_prefixes (index) {
      if (this.validation_errors.prefixes && this.validation_errors.prefixes.length > index) {
